 ...
 *
 * - Response to mixture of gases is set based on  datasheet.GAZEBO Gaden
 * -----------------------------------------------------------------------------------------------*/
...
int main( int argc, char** argv )
{
    ...
    ros::Publisher service_pub = n.advertise<simulated_gas_sensor::sim_sensor>("Sensor_response", 600);
	  ros::Publisher position_pub = n.advertise<simulated_gas_sensor::sim_sensor>("Sensor_pose", 600);
    ...
    while (ros::ok())
    {
        ...
        {
            //Current sensor pose
          simulated_gas_sensor::sim_sensor pose_pub;
            float x_pos = transform.getOrigin().x();
            float y_pos = transform.getOrigin().y();
            float z_pos = transform.getOrigin().z();
              pose_pub.position.position.x=x_pos;
              pose_pub.position.position.y=y_pos;
              pose_pub.position.position.z=z_pos;
                position_pub .publish( pose_pub); 
            ...
            if (client.call(srv))
            {
                ...
                {
                   ROS_INFO(" %s:%.4f at (%.2f,%.2f,%.2f)",srv.response.gas_type[i].c_str(), srv.response.gas_conc[i],srv.request.x, srv.request.y, srv.request.z );
                }
...
//////////////////////////////////////////////////////////*/
               ...
               //include sevice for fleet of UAVs to get sensor data from sensor

               simulated_gas_sensor::sim_sensor serv_data;
                serv_data.service_res = srv.response.gas_conc;
                for (int i=0; i<srv.response.gas_conc.size(); i++)
                {
              //ROS_INFO("gas conc to be publish: %.4f",  serv_data.service_res[i] );
                }
                for (int i=0; i<srv.response.gas_type.size(); i++)
                {
               // ROS_INFO("position to be publish %.2f, %.2f",  serv_data.position.position.x, serv_data.position.position.y );
                }
              ...
              //Publish simulated service data to fleet of UAVs
               service_pub.publish(serv_data);
